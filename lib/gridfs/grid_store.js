 ...
 * @param {string} [options.storage] Significates, which storage in geocluster use (it is used by sharding index)
 * @param {number} [options.hash] Hash to be used over mongodb md5
...
<<<<<<< HEAD
          if (self.uploadDate != null) {
            files.remove({
              '_id': self.fileId
            }, self.writeConcern, function(err, collection) {
              if (err && typeof callback == 'function') return callback(err);

              buildMongoObject(self, function(err, mongoObject) {
                if (err) {
                  if (typeof callback == 'function') return callback(err); else
                    throw err;
                }
          ...
          if(self.uploadDate != null) {
            buildMongoObject(self, function(err, mongoObject) {
              ...
              files.save(mongoObject, options, function(err) {
                if(typeof callback == 'function')
                  callback(err, mongoObject);
>>>>>>> b785aa0bcd1884f1a5729b09217b1dad29586d12
              });
            });
          } else {
...
var buildMongoObject = function(self, callback) {
  ...
  if (self.options.storage) {
    mongoObject.storage = self.options.storage;
  }
  if (self.options.hash) {
    mongoObject.md5 = self.options.hash;
    ...
    new GridStore(db, names, "w", options).open(function(err, gridStore) {
      ...
      deleteChunks(gridStore, function(err, result) {
        ...
        gridStore.collection(function(err, collection) {
          ...
          }, writeConcern, function(err, result) {
            callback(err, self);
          });
        });
      });
